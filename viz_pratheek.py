...
def _parse_line(line: str):
    ...
    try:
        ...
        return ts, stage, baro_alt, latitude, longitude, acc_z_val, vel_z_val, state_pz, motor_bat_voltage, comp_bat_voltage
...
def _on_notify(_, data: bytes):
    ...
    *lines, remainder = _line_buf.split(b"\n")
    ...
    for raw in lines:
        ...
        if parsed:
            ts, stage, alt, latitude, longitude, az, vz, pz, motor_bat, comp_bat = parsed
            print(f"[PARSED OK] t={ts:.2f} stage={stage} alt={alt:.2f} pz={pz:.2f} vz={vz:.2f} az={az:.2f} mbat={motor_bat:.2f}V cbat={comp_bat:.2f}V")
            t.append(ts)
...
async def _plot_updater(fig, axes, lines, anns, status_overlay, window_secs, running_flag):
    ...
    ln_alt_baro, ln_alt_state, ln_path, ln_current, ln_vel, ln_acc = lines
    ann_alt, ann_latlon, ann_vel, ann_acc = anns
...
async def main():
    ...
    async with BleakClient(dev) as client:
        ...
        # --- 4 stacked subplots: Alt, Map, Vel, Acc ---
        plt.ion()
        ...
        updater = asyncio.create_task(
            _plot_updater(fig, (ax1, ax2, ax3, ax4),
                          (ln_alt_baro, ln_alt_state, ln_path, ln_current, ln_vel, ln_acc),
                          (ann_alt, ann_latlon, ann_vel, ann_acc),
                          status_overlay,
                          WINDOW_SECS, running)
        )
