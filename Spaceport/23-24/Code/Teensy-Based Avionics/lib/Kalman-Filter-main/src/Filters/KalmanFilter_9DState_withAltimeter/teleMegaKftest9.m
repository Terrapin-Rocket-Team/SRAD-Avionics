...
initial_control = zeros(1,6);
initial_state = zeros(1,9);
P = eye(9)*500;
...
kf = MultiVarKalmanFilter(initial_state, P, time(2) - time(1), VarMat);
...
for i = 2:size(time)
    ...
    measurement = [x; y; z;accel_x(i); accel_y(i); accel_z(i); z];
