...
Si4464::Si4464(Si4464HardwareConfig hConfig, Si4464PinConfig pConfig)
{
    // update internal variables with hardware configuration
    this->mod = hConfig.mod;
    this->dataRate = hConfig.dataRate;
    this->freq = 220e6;
    this->channel = hConfig.channel;
...
bool Si4464::begin(const uint8_t *config, uint32_t length)
{
    // set the config before calling normal begin
    this->setRadioConfig(config, length);
    // call and return normal begin
    return this->begin();
}

bool Si4464::tx(const uint8_t *message, uint16_t len)
{
    return this->startTX(message, len, len);
}

bool Si4464::rx(uint8_t *data, uint16_t *len, uint16_t maxLen)
{
    // check for message
    if (!this->available)
    {
        if (this->state != STATE_ENTER_RX || this->state != STATE_RX)
        {
            this->startRx();
        }
        ...
        // while (!this->available && millis() - rxTimer < Si4464::RX_TIMEOUT)
        // {
        //     this->update();
        // }
    }
    // check if buffer is long enough and we got a message
    if (this->available && this->length <= maxLen)
    {
        // copy the message
        memcpy(data, this->buf, this->length);
        *len = this->length;
...
bool Si4464::startTX(const uint8_t *message, uint16_t len, uint16_t totalLen)
{
    // make sure the packet isn't too long and we have at least 1 byte
    if (totalLen > Si4464::MAX_LEN || len < 0)
    ...
    if (this->state == STATE_IDLE || this->state == STATE_RX || this->state == STATE_RX_COMPLETE)
    {
        ...
        this->availLen = len;
        this->xfrd = 0;
        memcpy(this->buf, message, this->availLen);
        ...
        while (count++ < FIFO_LENGTH - 2 && this->xfrd < this->availLen)
        {
            ...
            // Serial1.print((char)this->buf[this->xfrd - 1]);
        }
        // Serial1.println();
        ...
        // Serial1.print("TX chan ");
        // Serial1.println(this->channel);
        this->spi_write(C_START_TX, sizeof(txArgs), txArgs);
        // this->waitCTS();

        this->state = STATE_TX;
        // Serial1.println("4464 start tx");
        // delay(1);
        // Serial1.println(this->readFRR(0));
...
bool Si4464::startRx()
{
    ...
    if (this->state == STATE_IDLE)
    {
        ...
        // Serial1.print("RX chan ");
        // Serial1.println(this->channel);
        this->spi_write(C_START_RX, 7, rxArgs);
        this->state = STATE_RX;
        // Serial1.println("4464 start rx");
        return true;
    }
    return false;
}
...
void Si4464::handleRX()
{
    ...
    if (this->gpio3()) // valid preamble and more than RX_THRESH bytes in FIFO
    {
        // this->debugTimer = micros();
        // Serial1.println("here");
        // Serial1.println(this->xfrd);
        // Serial1.println(this->length);
        ...
        // for (int i = 0; i < sizeof(rClearFIFO); i++)
        //     Serial1.println(rClearFIFO[i]);
        if (rClearFIFO[0] < RX_THRESH)
        {
            this->RXFullFlag = true;
        }

        uint16_t bytesAvail = rClearFIFO[0];
        ...
        if (this->xfrd == 0)
        {
            ...
            if (this->length > Si4464::MAX_LEN || this->length == 0)
            {
                this->length = 0;
        ...
        int count = lenBytes;
        while (this->xfrd < this->length && count < bytesAvail - 2)
        {
            count++;
...
uint16_t Si4464::writeTXBuf(const uint8_t *data, uint16_t len)
{
...
uint16_t Si4464::readRXBuf(uint8_t *data, uint16_t len)
{
    // POSSIBLE BUG: This likely doesn't work, this->availLen will always be less than 0 for rx
...
int Si4464::RSSI()
{
    return this->rssi / 2 - 64 - 70;
} // magic formula from datasheet

// void Si4464::setModemConfig(Si4464Mod mod, Si4464DataRate dataRate, uint32_t freq)
// {
//     // set modulation
...
//     Si4464Band bandConfigs[6] = {BAND_150, BAND_225, BAND_300, BAND_450, BAND_600, BAND_900};
...
//         return; // Error: data rate is not part of the Si4464DataRate enum (should never happen)
//     }
...
//     // this->setProperty(G_MODEM, P_MODEM_RAW_SEARCH, 0x84);
//     this->setProperty(G_FREQ_CONTROL, P_FREQ_CONTROL_VCOCNT_RX_ADJ, 0xFE);
// }

void Si4464::setPower(uint8_t pwr)
{
...
void Si4464::setPacketConfig(Si4464Mod mod, uint8_t preambleLength, uint8_t preambleThreshold)
{
...
void Si4464::setPins(Si4464Pin gpio0Mode, Si4464Pin gpio1Mode, Si4464Pin gpio2Mode, Si4464Pin gpio3Mode, Si4464Pin irqMode, bool pullup)
{
...
void Si4464::setFRRs(Si4464FRR regAMode, Si4464FRR regBMode, Si4464FRR regCMode, Si4464FRR regDMode)
{
...
void Si4464::setTXThreshold(uint8_t size)
{
...
void Si4464::setRXThreshold(uint8_t size)
{
    // limit size to 64 bytes
    if (size > 64)
        size = 64;
...
void Si4464::setAFC(bool enabled)
{
    // get existing config
...
bool Si4464::shutdown(bool shutdown)
{
    if (shutdown)
    {
...
void Si4464::setProperty(Si4464Group group, Si4464Property start, uint8_t data)
{
...
void Si4464::getProperty(Si4464Group group, Si4464Property start, uint8_t &data)
{
...
void Si4464::setProperty(Si4464Group group, const uint8_t num, Si4464Property start, uint8_t *data)
{
...
void Si4464::getProperty(Si4464Group group, const uint8_t num, Si4464Property start, uint8_t *data)
{
...
void Si4464::setProperty(uint8_t *data, uint8_t size)
{
    digitalWrite(this->_cs, LOW);
...
void Si4464::readFRRs(uint8_t data[4], uint8_t start)
{
...
void Si4464::powerOn()
{
    ...
    uint8_t XTAL_OPTIONS = 0b00000000;
#else
...
void Si4464::waitCTS(uint32_t timeout)
{
    ...
    {
        Serial1.print("ERROR: CTS timeout");
    }
}

bool Si4464::checkCTS()
{
...
bool Si4464::CTS()
{
    ...
    Serial1.print("ERROR: CTS pin not configured: ");
    Serial1.println(this->_cts);
    return false;
}

void Si4464::sendCommand(Si4464Cmd cmd, uint8_t argcCmd, uint8_t *argvCmd, uint8_t argcRes, uint8_t *argvRes)
{
...
void Si4464::sendCommandR(Si4464Cmd cmd, uint8_t argcRes, uint8_t *argvRes)
{
...
void Si4464::sendCommandC(Si4464Cmd cmd, uint8_t argcCmd, uint8_t *argvCmd)
{
    // send the cmd with its args
    this->spi_write(cmd, argcCmd, argvCmd);
...
void Si4464::setRadioConfig(const uint8_t *config, uint32_t length)
{
...
// private methods
void Si4464::spi_write(uint8_t cmd, uint8_t argc, uint8_t *argv)
{
    // CS low through entire SPI command
    digitalWrite(this->_cs, LOW);
...
void Si4464::spi_read(uint8_t argc, uint8_t *argv)
{
    // CS low through entire SPI command
    uint8_t pos = 0;
    ...
    while (cts != 0xFF)
    {
        ...
        {
            Serial1.println("ERROR: spi_read(), CTS took too long");
            return;
        }
    }
...
void Si4464::from_bytes(uint16_t &val, uint8_t pos, uint8_t bytePos, uint8_t *arr, bool MSB)
{
...
void Si4464::from_bytes(uint32_t &val, uint8_t pos, uint8_t bytePos, uint8_t *arr, bool MSB)
{
...
void Si4464::from_bytes(uint64_t &val, uint8_t pos, uint8_t bytePos, uint8_t *arr, bool MSB)
{
...
void Si4464::to_bytes(uint16_t val, uint8_t pos, uint8_t bytePos, uint8_t *arr, bool MSB)
{
    int iterNum = 2 - bytePos;
    if (MSB)
    {
...
void Si4464::to_bytes(uint32_t val, uint8_t pos, uint8_t bytePos, uint8_t *arr, bool MSB)
{
    int iterNum = 4 - bytePos;
    if (MSB)
    {
...
void Si4464::to_bytes(uint64_t val, uint8_t pos, uint8_t bytePos, uint8_t *arr, bool MSB)
{
    int iterNum = 8 - bytePos;
    if (MSB)
    {
...
void Si4464::applyRadioConfig()
{
    ...
    {
        Serial1.println("ERROR: could not find correct header file for selected config");
    }
}

void Si4464::applyWDSConfig(bool applyDefault)
{
    if (applyDefault)
    {
