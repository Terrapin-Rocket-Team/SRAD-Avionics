...
bool MockRadio::tx(const uint8_t *message, uint16_t len)
{
    return this->startTX(message, len, len);
}

bool MockRadio::rx(uint8_t *data, uint16_t *len, uint16_t maxLen)
{
    // check for message
    if (!this->available)
    {
        ...
        while (!this->available && millis() - rxTimer < MockRadio::RX_TIMEOUT)
        {
            this->update();
        }
    }
