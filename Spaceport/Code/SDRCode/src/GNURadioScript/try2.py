...
class blk(gr.sync_block):
    ...
    def _read_some(self, nmax):
        if self._is_serial:
            return self._fh.read(min(nmax, self.chunk))
        else:
            if self._eof and not self.repeat:
                return b""
            if self.text:
                # Read chars, encode to bytes
                s = self._fh.read(min(nmax, self.chunk))  # chars, not bytes
                if not s:
                    if self.repeat:
                        try:
                            self._fh.seek(0)
                            s = self._fh.read(min(nmax, self.chunk))
                            self._eof = False
                        except Exception:
                            self._eof = True
                    else:
                ...
                buf = self._fh.read(min(nmax, self.chunk))
                if not buf:
                    if self.repeat:
                        try:
                            self._fh.seek(0)
    ...
    def work(self, input_items, output_items):
        ...
        produced = 0
        attempts = 0
        while produced < want and attempts < 4:
            chunk = self._read_some(want - produced)
            if chunk:
                n = len(chunk)
                out[produced:produced+n] = np.frombuffer(chunk, dtype=np.uint8, count=n)
                produced += n
            else:
                attempts += 1
                time.sleep(0.001)
