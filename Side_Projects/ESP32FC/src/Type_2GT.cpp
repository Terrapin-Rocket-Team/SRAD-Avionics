...
// Copied from Side_Projects/Radio_Code so both link ends share identical PHY.
// See Type_2GT.h. (STM32 #ifdef guards from the original are dropped here.)
...
int Type2GT::begin()
{
    ...
    // Keep PHY exactly aligned with the flight radio side.
...
int Type2GT::recieve()
{
    ...
    state = (lastReceiveRc == RADIOLIB_ERR_NONE) ? RAD_RX : RAD_IDLE;
    return lastReceiveRc;
}

int Type2GT::transmit(const char *str)
{
    // Blocking TX to guarantee packet completion before next send.
    state = RAD_TX;
...
void Type2GT::respondToIrq()
{
    if (state == RAD_TX)
    {
        state = RAD_IDLE;  // TX complete
    }
    else if (state == RAD_RX)
    {
        state = RAD_HAS_DATA;
    }
}
