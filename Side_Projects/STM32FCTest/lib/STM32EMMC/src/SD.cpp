/*
 ...
 This library provides four key benefits:
   ...
   * Including `STM32EMMC.h` automatically creates a global
     `EMMC` object which can be interacted with in a similar
...
/**
  ...
  */
bool EMMCClass::begin(uint32_t detect, uint32_t level)
{
  return beginRaw(detect, level) && mount();
}

bool EMMCClass::beginRaw(uint32_t detect, uint32_t level)
{
  if (_cardInitialized) {
    return true;
  }
...
bool EMMCClass::format(uint8_t formatFlags, uint32_t allocationUnit)
{
  if (!_cardInitialized) {
    return false;
  }

  if (_fatFs.format(formatFlags, allocationUnit)) {
    _mounted = true;
    return true;
  }
...
uint32_t EMMCClass::lastError(void) const
{
  return _card.errorCode();
}

int EMMCClass::lastFsError(void) const
{
  return static_cast<int>(_fatFs.lastError());
}

void EMMCClass::getCardInfo(BSP_SD_CardInfo *info) const
{
  _card.getCardInfo(info);
}
...
/**
  ...
  * @retval true or false
  */
bool EMMCClass::exists(const char *filepath)
{
  FILINFO fno;
...
/**
  ...
  */
bool EMMCClass::mkdir(const char *filepath)
{
...
/**
  ...
  * @retval true or false
  */
bool EMMCClass::rmdir(const char *filepath)
{
...
/**
  ...
  */
File EMMCClass::open(const char *filepath, uint8_t mode /* = FA_READ */)
{
  File file = File();
  ...
  if ((mode == FILE_WRITE) && (!EMMC.exists(filepath))) {
...
/**
  ...
  * @retval true or false
  */
bool EMMCClass::remove(const char *filepath)
{
  if (f_unlink(filepath) != FR_OK) {
    return false;
  } else {
    ...
    if ((fno.fattrib & AM_DIR) == 0) {
      ...
      if (flags & LS_R) {
        ...
        if (fullPath != NULL) {
          ...
          File filtmp = EMMC.open(fullPath);
...
File File::openNextFile(uint8_t mode)
{
  ...
  while (1) {
    ...
    if (fullPath != NULL) {
      ...
      File filtmp = EMMC.open(fullPath, mode);
      free(fullPath);
      return filtmp;
    } else {
