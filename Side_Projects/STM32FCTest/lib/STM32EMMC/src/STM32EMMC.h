...
#include "SdFatFs.h"
  ...
  #error "HAL_MMC_MODULE_ENABLED is required"
#endif
...
class EMMCClass {
  ...
  public:
    /* Initialize the eMMC peripheral */
    bool begin(uint32_t detect = SD_DETECT_NONE, uint32_t level = SD_DETECT_LEVEL);
    bool beginRaw(uint32_t detect = SD_DETECT_NONE, uint32_t level = SD_DETECT_LEVEL);
    bool mount(void);
    bool format(uint8_t formatFlags = FM_ANY | FM_SFD, uint32_t allocationUnit = 0U);
    /* Call this when the device is disconnected. It allows a fresh reinitialization later. */
    bool end(void);
    bool endRaw(void);
    uint32_t lastError(void) const;
    int lastFsError(void) const;
    void getCardInfo(BSP_SD_CardInfo *info) const;
