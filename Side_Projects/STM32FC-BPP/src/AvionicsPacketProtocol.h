...
constexpr int32_t kUnknownSigned32 = std::numeric_limits<int32_t>::min();
...
struct BppTelemetry
{
    ...
    bool hasGps = false;
    float gpsAltitudeFeet = 0.0f;
    bool hasGpsVelocity = false;
...
{
    return kPacketHeaderSize + bppTelemetryPayloadSize();
}

inline bool isKnownMessageType(uint8_t rawType)
{
    return rawType <= static_cast<uint8_t>(MessageType::BPPTELEM);
}
...
inline size_t fixedPayloadSize(MessageType type)
{
    ...
    case MessageType::BPPTELEM:
        return bppTelemetryPayloadSize();
    default:
        return 0;
    }
}
...
inline int32_t quantizeSigned32(float value, float scale)
{
    ...
    return static_cast<int32_t>(
        clampSigned(scaled, std::numeric_limits<int32_t>::min() + 1LL, std::numeric_limits<int32_t>::max()));
}
...
inline bool encodeBppTelemetry(const BppTelemetry &input, PacketBuffer &packet)
{
    uint8_t payload[bppTelemetryPayloadSize()] = {};
    size_t offset = 0;

    uint8_t flags = 0;
    if (input.hasBaroAltitude)
        flags |= 0x01;
    if (input.hasGps)
        flags |= 0x02;
    if (input.hasBattery)
        flags |= 0x04;
    if (input.hasBaroVelocity)
        flags |= 0x08;
    ...
    const int32_t baroAltitudeFeet = input.hasBaroAltitude
                                         ? quantizeSigned32(input.baroAltitudeFeet, 1.0f)
                                         : kUnknownSigned32;
    writeI32LE(&payload[offset], baroAltitudeFeet);
    offset += 4;

    const int32_t gpsAltitudeFeet = input.hasGps
                                        ? quantizeSigned32(input.gpsAltitudeFeet, 1.0f)
                                        : kUnknownSigned32;
    writeI32LE(&payload[offset], gpsAltitudeFeet);
    offset += 4;
    ...
    const int16_t baroVelocityCms = input.hasBaroVelocity ? quantizeSigned(input.baroVelocityZMs, 10.0f) : kUnknownSigned16;
    writeI16LE(&payload[offset], baroVelocityCms);
    offset += 2;
...
inline bool decodeBppTelemetry(const uint8_t *packet, size_t packetSize, BppTelemetry &output)
{
    if (!packetLengthLooksValid(packet, packetSize))
        return false;

    if (static_cast<MessageType>(packet[0]) != MessageType::BPPTELEM)
        return false;
    ...
    const int32_t baroAltitudeFeet = readI32LE(&payload[offset]);
    offset += 4;
    output.hasBaroAltitude = (flags & 0x01) != 0 && baroAltitudeFeet != kUnknownSigned32;
    output.baroAltitudeFeet = output.hasBaroAltitude ? static_cast<float>(baroAltitudeFeet) : 0.0f;

    const int32_t gpsAltitudeFeet = readI32LE(&payload[offset]);
    offset += 4;

    const uint16_t batteryCentivolts = readU16LE(&payload[offset]);
    offset += 2;
    output.hasBattery = (flags & 0x04) != 0 && batteryCentivolts != kUnknownBatteryCentivolts;
    output.batteryVolts = output.hasBattery ? static_cast<float>(batteryCentivolts) / 100.0f : 0.0f;
    ...
    output.hasGps = (flags & 0x02) != 0 &&
                    gpsAltitudeFeet != kUnknownSigned32 &&
                    latitudeE7 != kUnknownGpsE7 &&
                    longitudeE7 != kUnknownGpsE7;
    output.gpsAltitudeFeet = output.hasGps ? static_cast<float>(gpsAltitudeFeet) : 0.0f;
    output.latitudeDeg = output.hasGps ? static_cast<double>(latitudeE7) / 10000000.0 : 0.0;
    output.longitudeDeg = output.hasGps ? static_cast<double>(longitudeE7) / 10000000.0 : 0.0;

    const int16_t baroVelocityCms = readI16LE(&payload[offset]);
    offset += 2;
    output.hasBaroVelocity = (flags & 0x08) != 0 && baroVelocityCms != kUnknownSigned16;
    output.baroVelocityZMs = output.hasBaroVelocity ? static_cast<float>(baroVelocityCms) / 10.0f : 0.0f;

    const int16_t gpsVelocityCms = readI16LE(&payload[offset]);
    offset += 2;
